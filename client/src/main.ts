...
const main = async (): Promise<void> => {
   ...
   */
  setBootStatus('Building the meadow…');
  await paint();
  ...
  setBootStatus('Loading player…');
  game.loadingStep('Loading player…');
  await paint();
  ...
  if (clientConfig.debug) {
    ...
    (window as Window & { __money?: DebugHandle }).__money = { game, loop };
  }

  // Hidden only on a REAL join. Cash, levels, Wins and upgrades are all
