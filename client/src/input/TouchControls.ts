...
/**
 * Touch controls: a virtual stick, a JUMP buttonand
 * drag-to-look.
 *
 ...
 * so the stick's rest position moves right by the rail's lane. The lane is
 * the rail's full width, computed by the HUD stylesheet from the same unit
 * the rail is sized with, so the two can never overlap whatever the size.
 */
@media (orientation: landscape) and (max-height: 500px) {
  .aoe-touch__stick {
    left: calc(var(--aoe-safe-l, 0px) + 26px + var(--aoe-rail-lane, 110px) + var(--aoe-stick-radius));
  }
}
`;
