...
- **The money ball is made in the meadow.** `PlayerState.ballActive` (replicated, never persisted) is set only by `CashService` when a meadow stride is credited, and cleared by a death, a placement, a rebirth and a join. The simulation loads the crossing supply from the wallet only through an active ball (`BridgeState.active`), so the meadow starts every run; the client draws no ball while it is inactive and shrinks the ball by the square root of the supply left while crossing.
- **The course is Spawn → Lava → Island → Lava → Island…**, exactly 30 stages (`STAGE_COUNT`): the first six hand-tuned, then ×1.8 cost and ×1.6 Wins per stage with the six biomes cycling; no obstacles, parkour, mazes or checkpoints. Lava is crossed only by paying from the crossing supply; jumping cannot bypass it (`verify:course` proves both). Reaching an island drops the bridge and the supply; a starved player is dragged to a stop, pulled into the lava, dies, and respawns with their bills intact. Wins are never touched by any of that.
...
- **No Render, no Netlify.** Deploy is the Dockerfile + `.github/workflows/deploy.yml` for Bloxity: game id `money-roll`, `dev` branch → dev channel, `main` → prod, one secret `LEGION_DEPLOY_TOKEN`, version = commit SHA.
...
- **One stud pitch.** `STUD_TILE` in `client/src/world/texturedBox.ts` (6 units per tile of four studs, the spawn floor's) is what every builder tiles stud textures at through `texturedBox`; never use a plain `BoxGeometry` or a different tile for a studded surface, or its squares will be a different size from the floor's.
- All textures are drawn on canvas at runtime (`client/src/world/WorldTextures.ts`): `studs` for floors, pads, props and hills (props go through `MoneyWorld.propMaterial`, their outline is `shade(colour, 0.68)`), `brick` for the block walls, `bills` for money, `lava`. `money.mp3` is the ball's rolling loop (`AudioManager.setRolling`). The only image files are the player texture and the nine HUD icons in `assets/ui/` (Bills, Pets, Sound, Upgrades, trophy, rebirth, aura, shoe, shop), used as supplied.
