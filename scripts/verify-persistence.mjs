...
/** Earn some cash the only way there is: by WALKING THE MEADOW. Returns the new total. */
const earn = async (room, seconds = 1.2) => {
  const before = me(room).lifetimeCash;
  let seq = room.nextSeq ?? 1;
  // Spawn is a dozen units short of the meadow, so every walk covers that first.
  const steps = Math.round(Math.max(seconds, 1.8) / 0.05);
  for (let i = 0; i < steps; i += 1) {
    room.send('move', { seq: seq++, dt: 0.05, moveX: 0, moveZ: 1, jump: false, cameraYaw: 0 });
    await sleep(50);
  }
  room.nextSeq = seq;
  await waitFor(() => me(room).lifetimeCash > before, 'cash to be credited', 8000);
  // Back to spawn, so the next stretch starts beside the meadow again.
  room.send('requestRespawn', {});
  await sleep(150);
  return me(room).lifetimeCash;
};
...
const PROGRESS_KEYS = [
  'cash', 'levelCash', 'lifetimeCash', 'wins', 'ownedBills', 'rebirths', 'ownedAuras', 'auraSlot',
  'speedUpgrades', 'petSlotUpgrades', 'bestStage', 'playSeconds', 'pets', 'equippedPets',
];
...
class Mongod {
  constructor(binary, dbPath, port) {
    ...
    this.dbPath = dbPath;
    this.port = port;
    this.uri = `mongodb://127.0.0.1:${port}/money_verify`;
  }
  async start() {
...
const runSuite = async (ctx) => {
  ...
  console.log(`\n${kind}: guests`);
  {
    ...
    const stored = await waitForProfile(reader, 'p_guest1', (p) => p.lifetimeCash >= t1, 'guest save');
    check(stored.displayName === 'GuestOne', 'the guest save keeps its display name');
    const again = await joinAs(port(), { playerId: 'p_guest1' });
    check(me(again).lifetimeCash === stored.lifetimeCash, `existing guest profile restores (lifetimeCash ${stored.lifetimeCash})`);
  ...
  console.log(`\n${kind}: first login migrates`);
  {
    ...
    check(me(s).lifetimeCash === before.lifetimeCash, 'the session carries the guest progress into the account');
    ...
    check(guest.migratedSnapshot?.lifetimeCash === before.lifetimeCash, 'the guest copy keeps its progress as migratedSnapshot');
    check(guest.lifetimeCash === 0, 'the retired guest profile is fresh');
    const more = await earn(s);
    check(more > before.lifetimeCash, 'the signed-in session keeps earning');
    await leaveAndWait(s);
    const after = await waitForProfile(reader, acctKey, (p) => p.lifetimeCash >= more, 'account save after playing');
    check((await profileOf(reader, 'p_guest1')).lifetimeCash === 0, 'account play does not touch the retired guest');
    ctx.accountTotal = after.lifetimeCash;
  ...
  console.log(`\n${kind}: nothing leaks to a claimed id`);
  {
    ...
    check(me(raw).lifetimeCash === 0, 'a raw account id as a browser id gets a fresh guest, nothing more');
    await leaveAndWait(raw);
    const forged = await joinAs(port(), { playerId: 'p_forger', token: `forged:${acct}` });
    check(me(forged).lifetimeCash === 0, 'a forged token gets a fresh guest, nothing more');
    await leaveAndWait(forged);
    const noid = await joinAs(port(), { playerId: 'p_noid', token: `noid:${acct}` });
    check(me(noid).lifetimeCash === 0 && server.current.log.includes('carried no usable _id'), 'a 2xx without an _id does not verify');
  ...
  {
    const s = await joinAs(port(), { playerId: 'p_guest1', token: `valid:${acct}` });
    check(me(s).lifetimeCash === ctx.accountTotal, 'reconnecting as the same account restores it');
    const out = await setAuth(s, null, 'guest');
    check(out.status === 'guest' && (await settled(s, (p) => p.lifetimeCash === 0, 'fresh guest state')), 'logout -> guest, fresh because this browser was migrated');
    const guestBit = await earn(s, 0.8);
    const back = await setAuth(s, `valid:${acct}`, 'account');
    check(back.status === 'account' && (await settled(s, (p) => p.lifetimeCash === ctx.accountTotal, 'account state')), 'signing back in restores the account');
    // The switch saved the guest it LEFT from live state before loading the account.
    const guest = await waitForProfile(reader, 'p_guest1', (p) => p.lifetimeCash >= guestBit, 'the leaving guest save');
    check(guest.migratedTo === acctKey && guest.lifetimeCash >= guestBit, 'the retired guest is not migrated a second time');
    check((await profileOf(reader, acctKey)).lifetimeCash === ctx.accountTotal, 'guest play never reached the account');
    await leaveAndWait(s);
  }
  ...
  console.log(`\n${kind}: the same account elsewhere`);
  {
    ...
    check(me(other).lifetimeCash === ctx.accountTotal, 'the same account on another browser sees the same progress');
  ...
  console.log(`\n${kind}: an existing account is never overwritten`);
  {
    ...
    await waitForProfile(reader, 'p_guest3', (p) => p.lifetimeCash >= t3, 'guest3 save');
    const s = await joinAs(port(), { playerId: 'p_guest3', token: `valid:${acct}` });
    check(me(s).lifetimeCash === ctx.accountTotal, 'the account wins over the browser profile');
    const guest3 = await profileOf(reader, 'p_guest3');
    check(guest3.lifetimeCash >= t3 && !guest3.migratedTo, 'the browser profile is left untouched and not retired');
    const out = await setAuth(s, null, 'guest');
    check(out.status === 'guest' && (await settled(s, (p) => p.lifetimeCash === guest3.lifetimeCash, 'guest3 state')), "logout returns that browser's own progress");
    await leaveAndWait(s);
  }
  ...
  console.log(`\n${kind}: Bloxity unavailable`);
  {
    ...
    check(me(s).lifetimeCash === 0 && server.current.log.includes('bloxity unavailable, will re-ask'), 'a token Bloxity cannot answer for is let in as a guest and re-asked later');
    await leaveAndWait(s);
    const h = await joinAs(port(), { playerId: 'p_outage2', token: 'neterr:acct9' });
    check(me(h).lifetimeCash === 0, 'a network error verifying is a guest too');
  ...
  console.log(`\n${kind}: a server restart loses nothing`);
  {
    ...
    await leaveAndWait(g);
    await waitForProfile(reader, 'p_restart', (p) => p.lifetimeCash >= t, 'save before restart');
    const accountBefore = await profileOf(reader, acctKey);
    await restart();
    const again = await joinAs(port(), { playerId: 'p_restart' });
    check(me(again).lifetimeCash >= t, 'a guest profile survives a server restart');
    await leaveAndWait(again);
    const s = await joinAs(port(), { playerId: 'p_browser2', token: `valid:${acct}` });
    check(me(s).lifetimeCash === accountBefore.lifetimeCash && me(s).wins === accountBefore.wins, 'an account profile survives a server restart');
    await leaveAndWait(s);
  }
  ...
  console.log(`\n${kind}: the wrong game`);
  {
    ...
    check(me(s).lifetimeCash === 0 && server.current.log.includes('rejected (401 GAME_TOKEN_INVALID)'), 'a token for another game slug is rejected by the verify route');
    await leaveAndWait(s);
    await server.current.stop();
    server.current = await ctx.startServer();
  }
...
const runJsonExtras = async (ctx, server) => {
  ...
  console.log('\njson: a corrupt file is moved aside');
  {
    ...
    await earn(g, 0.6);
    await leaveAndWait(g);
    await waitForProfile(reader, 'p_after_corrupt', (p) => p.lifetimeCash > 0, 'a save after the move-aside');
...
const runMongoExtras = async (ctx, server, mongod) => {
  ...
  console.log('\nmongo: the database goes down');
  {
    ...
    const stayed = stay.status === 'account' && /staying/.test(stay.note ?? '') && me(s).lifetimeCash === total;
    check(stayed, 'a sign-out that cannot reach storage stays on the account');
    if (!stayed) {
      note(`authState=${JSON.stringify(stay)} total=${total} now=${me(s).lifetimeCash}`);
    ...
    const landed = await waitForProfile(reader, acctKey, (p) => p.lifetimeCash >= more, 'the queued save to land once the database is back', 40_000);
    check(landed.lifetimeCash >= more, 'a save made during the outage lands once the database is back');
    const again = await joinAs(port(server), { playerId: 'p_browser2', token: `valid:${acct}` });
    check(me(again).lifetimeCash === landed.lifetimeCash && me(again).wins === ctx.accountWins, 'everything is intact after the outage');
    const out = await setAuth(again, null, 'guest');
    const wentThrough = out.status === 'guest' && (await settled(again, (p) => p.lifetimeCash === 0, 'guest after outage'));
    check(wentThrough, 'and the sign-out goes through now');
    if (!wentThrough) {
      note(`authState=${JSON.stringify(out)} now=${me(again).lifetimeCash} guestDoc=${JSON.stringify(await profileOf(reader, 'p_browser2'))}`);
      for (const line of server.current.lines.slice(-12)) note(line);
    }
    await leaveAndWait(again);
  }
  ...
  console.log('\nmongo: the legacy JSON file is imported, insert-only');
  {
    ...
    const legacy = {
      legacy_player: { ...ctx.guestProgress, lifetimeCash: 777, wins: 7, displayName: 'Legacy', updatedAt: 1 },
      [acctKey]: { ...account, lifetimeCash: 1, wins: 1, displayName: 'MUST NOT WIN', updatedAt: Date.now() + 1e9 },
    };
    ...
    check(imported?.lifetimeCash === 777 && imported?.wins === 7, 'a profile only the legacy file knew is added');
    const kept = await profileOf(reader, acctKey);
    check(kept.lifetimeCash === account.lifetimeCash && kept.displayName !== 'MUST NOT WIN', 'an existing account profile is never overwritten by the import');
...
const run = async () => {
  ...
  if (process.env.ONLY !== 'mongo') {
    const dataDir = mkdtempSync(path.join(tmpdir(), `money-json-${stamp}-`));
  ...
  if (!uri) {
    ...
    if (binary) {
      mongod = new Mongod(binary, mkdtempSync(path.join(tmpdir(), `money-mongod-${stamp}-`)), MONGO_PORT);
  ...
  if (!uri) {
    ...
    const dataDir = mkdtempSync(path.join(tmpdir(), `money-mongo-${stamp}-`));
