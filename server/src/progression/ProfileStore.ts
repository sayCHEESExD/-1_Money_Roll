...
class ProfileStore {
   ...
   * there is none. Only the DERIVING facts: level, the movement profile, the
   * equipped bill and the pet boost are recomputed by the services, which the
   * room re-runs in its join order right after this.
   */
  applyTo(player: PlayerState, profile: StoredProfile | null, keepIdentity = false): void {
    ...
    player.cash = p.cash;
    player.levelCash = p.levelCash;
    player.lifetimeCash = p.lifetimeCash;
    player.wins = p.wins;
    player.ownedBills = p.ownedBills | INITIAL_OWNED_BILLS;
    player.rebirths = p.rebirths;
    player.ownedAuras = p.ownedAuras;
    player.auraSlot = p.auraSlot;
    player.speedUpgrades = Math.min(255, p.speedUpgrades);
    player.petSlotUpgrades = Math.min(255, p.petSlotUpgrades);
    player.bestStage = p.bestStage;
    player.playSeconds = p.playSeconds;
    player.pets = p.pets;
    player.equippedPets = p.equippedPets;
    if (!keepIdentity) {
...
/** What a brand-new player holds: the free starter bill and nothing else. */
const freshProgress = (): ProgressFields => ({
  ...emptyProgress(),
  ownedBills: INITIAL_OWNED_BILLS,
});
